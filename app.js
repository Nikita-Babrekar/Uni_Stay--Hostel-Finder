...
// ==============================================================
// 🗺️ MAP ENGINE ARCHITECTURE (FOR DASHBOARD)
// ==============================================================
function initMap() {
    const mapElement = document.getElementById('map');
    if (mapElement && !map) {
        map = L.map('map').setView([28.6892, 77.2144], 12); // Default Delhi coordinates
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
            attribution: '&copy; OpenStreetMap contributors'
        }).addTo(map);
...
async function fetchHostels() {
    ...
    // Automatically checks if we are on the hostel-detail page
    if (document.getElementById('detailName')) {
        renderHostelDetails();
    }
}
...
function renderGlobalHostels(hostelsList) {
    ...
    hostelsList.forEach(hostel => {
        ...
        if (map && markerLayer) {
            const lat = parseFloat(hostel.latitude) || 28.6892;
            const lng = parseFloat(hostel.longitude) || 77.2144;
            mapBounds.push([lat, lng]);
...
window.applyAdvancedFilters = function() {
    ...
    const acSelect = document.getElementById('acFilter');
    if (acSelect && acSelect.value !== 'all') {
        const targetAc = acSelect.value.toLowerCase();
        filtered = filtered.filter(h => h.ac && h.ac.toLowerCase().includes(targetAc));
    }

    const bedSelect = document.getElementById('bedFilter');
    if (bedSelect && bedSelect.value !== 'all') {
        const targetBed = bedSelect.value.toLowerCase();
        filtered = filtered.filter(h => h.beds && h.beds.toLowerCase().includes(targetBed));
    }
...
// ==============================================================
// 📋 HOSTEL DETAILS SPECIFICATION VIEWPORT (CSV MAPPED)
// ==============================================================
function renderHostelDetails() {
    const urlParams = new URLSearchParams(window.location.search);
    const hostelId = urlParams.get('id');
    ...
    // Inject values strictly matching your CSV headers
    if (document.getElementById('detailName')) document.getElementById('detailName').innerText = hostel.name;
    if (document.getElementById('detailLocation')) document.getElementById('detailLocation').innerText = hostel.address;
    if (document.getElementById('detailPrice')) document.getElementById('detailPrice').innerText = `₹${parseInt(hostel.price || 0).toLocaleString()}/month`;
    
    if (document.getElementById('detailWater')) document.getElementById('detailWater').innerText = hostel.water_quality || 'RO Purified';
    if (document.getElementById('detailMess')) document.getElementById('detailMess').innerText = hostel.mess_quality || 'Available';
    
    // Inject custom structural elements for your CSV description placeholder if needed
    if (document.getElementById('detailDescription')) {
        document.getElementById('detailDescription').innerText = `Welcome to ${hostel.name}. A premium ${hostel.type} residential stay offering fully structured ${hostel.ac} spaces featuring ${hostel.beds} accommodations. Experience top tier security management under professional gate timings arranged as ${hostel.gate_timings || '06:00 AM to 10:00 PM'}.`;
    }

    // Set map coordinates globally so HTML inline Leaflet script can access it instantly
    window.currentActiveHostelLat = parseFloat(hostel.latitude) || 28.6892;
    window.currentActiveHostelLng = parseFloat(hostel.longitude) || 77.2144;
    ...
    if (blueprintsContainer) {
        const blueprints = [
            { label: "Room Size", value: hostel.room_size || '180 sq ft', icon: "📐" },
            { label: "Bed Type Structure", value: hostel.bed_type || 'Simple Iron Single', icon: "🛏️" },
            { label: "AC Specifications", value: hostel.ac || 'Non-AC', icon: "❄️" },
            { label: "Mattress Thickness", value: hostel.mattress_thickness || '4-Inch Coir', icon: "🩹" },
            { label: "Study Space Layout", value: hostel.study_space || 'Basic Desk', icon: "📝" },
            { label: "Power Outlets Available", value: hostel.power_outlets || '2 Ports', icon: "🔌" },
            { label: "Wardrobe Configuration", value: hostel.wardrobe_units || 'Steel Almirah', icon: "🚪" },
            { label: "Ventilation Framework", value: hostel.ventilation || 'Standard Window', icon: "🪟" },
            { label: "Washroom Layout Specs", value: hostel.washroom_spec || 'Attached Style', icon: "🚿" },
            { label: "Gate Timings Boundary", value: hostel.gate_timings || '10:00 PM', icon: "🔑" }
        ];
    ...
    // Fixed ID mapping for Staff Personnel Container inside detail page
    const staffContainer = document.getElementById('staffPersonnelContainer');
    if (staffContainer) {
        let staffHtml = '';
        if (hostel.staff_roster && hostel.staff_roster.trim() !== '') {
            let cleanStaff = hostel.staff_roster.replace(/[\[\]\"]/g, '').trim();
            let staffMembers = cleanStaff.split(',');

            staffHtml = staffMembers.map(member => {
                let parts = member.split(':');
                let role = parts[0] ? parts[0].trim() : 'Staff';
                let details = parts[1] ? parts[1].trim() : 'Assigned Personnel';
                let initial = role.charAt(0).toUpperCase();
                return `
                    <div class="staff-profile-card">
                        <div class="staff-avatar">${initial}</div>
                        <div class="spec-info">
                            <span class="spec-label" style="font-size:0.75rem; letter-spacing:1px;">${role}</span>
                            <span class="spec-value" style="font-size:0.95rem;">${details}</span>
                        </div>
                    </div>
                `;
            }).join('');
            ...
            staffHtml = `
                <div style="grid-column: 1/-1; padding: 1rem; color: var(--text-muted); font-size: 0.9rem;">
                    No assigned administrative staff roster cataloged.
                </div>
            `;
        }
...
function renderOwnerInventoryManagement() {
    ...
    if (ownerHostels.length === 0) {
        managerGrid.innerHTML = `
            <div style="grid-column: 1/-1; text-align: center; padding: 4rem; border: 1px dashed var(--border); border-radius: 12px; width:100%;">
                <p style="color: var(--text-muted); font-size: 1.1rem; margin: 0;">No active properties found.</p>
            </div>
        `;
        return;
    }
